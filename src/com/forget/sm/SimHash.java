...
public class SimHash {
	...
	public BigInteger simHash() throws IOException {
		...
		// 当为true时，分词器进行最大词长切分
		List<String> segment = KeywordSegment.segment(this.tokens, itemColl, null, 4, false, false);
		String word = null;
		Iterator<String> iterator = segment.iterator();
		while (iterator.hasNext()) {
			...
			for (int i = 0; i < this.hashbits; i++) {
				...
				if (t.and(bitmask).signum() != 0) {
					...
					if(keyItemColl.containsKey(word)){
						v[i] += -Math.log(keyItemColl.get(word).getValidCount()/dLogTotlCount);
					}else{
						v[i] += 1;
					}
				} else {
	...
	public static void main(String[] args) throws IOException {
		...
		int dis = SimHash.getDistance(hash1.strSimHash, hash2.strSimHash);
		System.out.println(hash1.hammingDistance(hash2) + " " + dis);
		// 根据鸽巢原理（也成抽屉原理，见组合数学），如果两个签名的海明距离在 3 以内，它们必有一块签名subByDistance()完全相同。
		int dis2 = SimHash.getDistance(hash1.strSimHash, hash3.strSimHash);
